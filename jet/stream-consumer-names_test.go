...
func TestStreamConsumerNamesOk(t *testing.T) {
	...
	(&sliptest.Function{
		Source: fmt.Sprintf(`(let* ((js (jet-connect :url %q :user "u1" :password "password"))
                                    ...
                                    (found (send jss :consumer-names :timeout 0.1)))
                              (send js :close)
                              found)`, natsURL),
		Expect: `("eater")`,
	}).Test(t)
...
func TestStreamConsumerNamesError(t *testing.T) {
	options := nats.Options{
		Url:      natsURL,
		User:     "u1",
		Password: "password",
	...
	(&sliptest.Function{
		Source: fmt.Sprintf(`(let* ((js (jet-connect :url %q :user "u1" :password "password"))
                                    (jss (send js :create-stream "list-test" :subjects '("test.list.>"))))
                              (send js :close)
                              (send jss :consumer-names))`, natsURL),
