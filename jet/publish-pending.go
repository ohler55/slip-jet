...
func (caller publishPendingCaller) Call(s *slip.Scope, args slip.List, depth int) slip.Object {
	self := s.Get("self").(*flavors.Instance)
	slip.MethodArgCountCheck(s, depth, self, ":publish-pending", len(args), 0, 0)
	cl := self.Any.(*Client)
