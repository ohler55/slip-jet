...
type streamPauseConsumerCaller struct{}

func (caller streamPauseConsumerCaller) Call(s *slip.Scope, args slip.List, depth int) slip.Object {
	self := s.Get("self").(*flavors.Instance)
	slip.MethodArgCountCheck(s, depth, self, ":pause-consumer", len(args), 2, 4)
	stream := self.Any.(jetstream.Stream)

	consumer := slip.MustBeString(args[0], "consumer")
	until, ok := args[1].(slip.Time)
	if !ok {
	...
	cpr, err := stream.PauseConsumer(ctx, consumer, time.Time(until))
	if err != nil {
		panic(err)
	}
...
func (caller streamPauseConsumerCaller) FuncDocs() *slip.FuncDoc {
	return &slip.FuncDoc{
		Name: ":pause-consumer",
