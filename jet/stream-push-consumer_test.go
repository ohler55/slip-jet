...
func TestStreamPushConsumerOk(t *testing.T) {
	...
	(&sliptest.Function{
		Source: fmt.Sprintf(`(let* ((js (jet-connect :url %q :user "u1" :password "password"))
                                    ...
                                    (consumer (send jss :create-push-consumer
                                                        ...
                                                        :timeout 0.1))
                                    (found (send jss :push-consumer "puss" :timeout 0.1))
                                    (not-found (send jss :push-consumer "no-one")))
                              (send js :close)
                              (list not-found
...
func TestStreamPushConsumerError(t *testing.T) {
	defer cleanupTestStream("get-test")

	(&sliptest.Function{
		Source: fmt.Sprintf(`(let* ((js (jet-connect :url %q :user "u1" :password "password"))
                                    (jss (send js :create-stream "get-test" :subjects '("test.get.>"))))
                              (send jss :push-consumer "get bad"))`, natsURL),
