...
type streamPushConsumerCaller struct{}

func (caller streamPushConsumerCaller) Call(s *slip.Scope, args slip.List, depth int) slip.Object {
	self := s.Get("self").(*flavors.Instance)
	slip.MethodArgCountCheck(s, depth, self, ":push-consumer", len(args), 1, 3)
	...
	consumer, err := stream.PushConsumer(ctx, name)
	if err != nil {
...
func (caller streamPushConsumerCaller) FuncDocs() *slip.FuncDoc {
	return &slip.FuncDoc{
		Name: ":push-consumer",
